...
export class ApiService {
  baseUrl: string = 'https://localhost:7182/api/Library/';
  ...
  getUserInfo(): User | null {
    ...
    var user: User = {
      ...
      MobNumber: decodedToken.mobileNumber,
        ...
        map((orders) => {
          let newOrders = orders.map((order: any) => {
            let newOrder: Order = {
              id: order.id,
              userId: order.userId,
              userName: order.user.FName + ' ' + order.user.lastName,
              bookId: order.bookId,
  ...
  getFine(order: Order) {
    ...
    return 1;//the order is not overdue.
  }
//   getFine(order: Order) {
//     let today = new Date();
//     let orderDate = new Date(Date.parse(order.orderDate));
//     let dueDate = new Date(orderDate);
//     dueDate.setDate(dueDate.getDate() + 10); // Assuming 10 days is the due date
    
//     if (today.getTime() > dueDate.getTime()) {
//         let diff = today.getTime() - dueDate.getTime();
//         let daysOverdue = Math.floor(diff / (1000 * 86400));
//         return daysOverdue * 50; // Assuming $50 fine per day
//     }
...
// }
  ...
  //   });
  // }
  returnBook(userId: string, bookId: string, fine: number | null) {
    let params = new HttpParams()
      .append('userId', userId)
      .append('bookId', bookId);
  
    if (fine !== null) {
      params = params.append('fine', fine.toString()); // Convert fine to string
    }
